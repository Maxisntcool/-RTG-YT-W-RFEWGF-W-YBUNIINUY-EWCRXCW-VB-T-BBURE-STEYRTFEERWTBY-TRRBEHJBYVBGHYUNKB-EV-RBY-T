...
local Commands = {
	...
	{"getrap",{"getrap","rap"},1,function(player,args)
		...
		for i,v in pairs(getPlayer(player,arg1)) do
			...
			sayremote:FireServer(string.format("%s has %s RAP",v.Name,GetTotalRAP(game.Players:GetUserIdFromNameAsync(v.Name))), "All")
	...
	{"emote",{"e","emote"},3,function(player,args)
		...
		if game.Players.LocalPlayer.Character.Humanoid.RigType == Enum.HumanoidRigType.R15 then
			...
			local function Moved()
				if game.Players.LocalPlayer.Character.Humanoid.MoveDirection ~= VectorZero and animationdebounce == true then
					for i,v in pairs(game.Players.LocalPlayer.Character.Humanoid:GetPlayingAnimationTracks()) do
						v:Stop()
					end
				end
...
function exeCommand(msg,plr)
	...
	local args = string.split(msg,"/") -- This is the subfix
...
Players.PlayerAdded:Connect(function(p)
	...
	spawn(function()
		...
		if getRank(p.Name) >= 3 then
			...
			sayremote:FireServer("/w "..p.Name.." ".."Welcome, "..p.Name.."! Your rank is "..getRank(p.Name), "All")
		end
	end)
end)
...
SetHats()
game.Players.LocalPlayer.CharacterAdded:Connect(SetHats)
Notification(game.Players.LocalPlayer.PlayerGui,"Bot Loaded!")
